...
public class SysResourceController extends AbstractBaseController {
    ...
    public Object list(@RequestParam(name="pageNumber", defaultValue="1")	Integer pageNumber, 
		...
		r.setStatus("1");
		r.setInfo("所有资源权限列表");
		//返回前台数据包装示例 也可以使用函数表达式依您喜好
		GeneralReturnData wrapList = R.wrapList(r, (fieldName, fildValue, wrapHashMap) -> {
      if("resourceType".equals(fieldName) && fildValue.equals("1")){
        wrapHashMap.put("resourceTypeText", "菜单");
      }
      if("resourceType".equals(fieldName) &&fildValue.equals("2")){
        wrapHashMap.put("resourceTypeText", "按钮");
      }
    });
		return wrapList;
    }
